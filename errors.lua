...
local maxClevel = 256    -- LUAI_MAXCCALLS (in llimits.h)

local function testrep (init, rep, close, repc, okhigh)
  local s = init .. string.rep(rep, maxClevel - 10) .. close ..
               string.rep(repc, maxClevel - 10)
  assert(load(s))   -- 190 levels is OK
  s = init .. string.rep(rep, maxClevel + 1) .. close ..
                string.rep(repc, maxClevel + 1)
  if not okhigh then
    checkmessage(s, "not enough registers")
  end
end

testrep("local a; a", ",a", "= 1", ",1")    -- multiple assignment
testrep("local a; a=", "{", "0", "}")
testrep("local a; a=", "(", "2", ")", true)
testrep("local a; ", "a(", "2", ")")
testrep("", "do ", "", " end", true)
testrep("a=false;", "while a do ", "", " end", true)
testrep("local a=true; ", "if a then else ", "", " end", true)
testrep("", "function foo () ", "", " end", true)
testrep("local a=''; a=", "a..", "a", "", true)
testrep("local a=1; a=", "a^", "a", "", true)
